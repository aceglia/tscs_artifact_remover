...
from star_emg.processing_utils import median_frequency, robust_max_percentile
...
if __name__ == "__main__":
    ...
    channel = 0
    output = star_emg.process_all_data(
        hankel_size=h_size,
        randomized=False,
        channel_idxs=[channel],
        # data_window=[40000, 60000],
        # data_window=[star_emg.get_init_signal().shape[-1] - 50000, star_emg.get_init_signal().shape[-1]-10000],
        threads=1,
    ...
    dic_to_save = {
        "output": output,
        "init_signal": star_emg.streamer.data_loader.apply_filtering(star_emg.get_init_signal())[channel, :],
        "data_rate": star_emg.streamer.data_loader.data_rate,
    }
    ...
    init_data = star_emg.streamer.data_loader.apply_filtering(star_emg.get_init_signal())[-1, :]
    # quality = Quality((1, 1, init_data.shape[-1]))
    # quality.compute_quality(np.asarray(init_data).astype(np.float64)[None, None], output.astype(np.float64)[None],  fs=star_emg.streamer.data_loader.data_rate)
    colors = ('k', 'purple')
    ...
    freq = rfftfreq(init_data.shape[-1], d=1 / star_emg.streamer.data_loader.data_rate)
    ...
    plt.vlines(
        median_frequency(init_data, star_emg.streamer.data_loader.data_rate),
        plt.ylim()[0],
        plt.ylim()[1],
        colors=colors[0],
        linestyles="--",
    ...
    plt.vlines(
        median_frequency(output[0, :], star_emg.streamer.data_loader.data_rate),
        plt.ylim()[0],
        plt.ylim()[1],
        colors=colors[1],
        linestyles="--",
