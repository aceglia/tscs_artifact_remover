...
if __name__ == "__main__":
    ...
    dic_to_save = {
        "output": output,
        "init_signal": artefact_remover.streamer.data_loader.apply_filtering(artefact_remover.get_init_signal())[channel, :],
    ...
    init_data = artefact_remover.streamer.data_loader.apply_filtering(artefact_remover.get_init_signal())[-1, :]
    # quality = Quality((1, 1, init_data.shape[-1]))
    # quality.compute_quality(np.asarray(init_data).astype(np.float64)[None, None], output.astype(np.float64)[None],  fs=artefact_remover.streamer.data_loader.data_rate)
    colors = ('k', 'purple')
    ...
    plt.vlines(
        median_frequency(output[0, :], artefact_remover.streamer.data_loader.data_rate),
        plt.ylim()[0],
        plt.ylim()[1],
        colors=colors[1],
        linestyles="--",
