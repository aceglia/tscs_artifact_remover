...
class Analysis:
    """
    Class for analysis from solution object given by the remover.
    """

    def __init__(
        self,
        compute_signal_error: bool = True,
        compute_frequency_analysis: bool = True,
        average_batch: bool = False,
        average_channels: bool = False,
        data_rate: float = 2000,
    ) -> None:
        """
        Initialize class
        ...
        Parameters
        ----------
        compute_signal_error: bool
            Whether to compute signal error (correlation, lag, rmse)
        compute_frequency_analysis: bool
            Whether to compute median frequency analysis
        average_batch: bool
            Return the analysis for the averaged batch
        average_channels: bool
            Return the analysis for the averaged channels
        data_rate: float
    ...
    def _average_over_defined_axes(self, data: np.ndarray) -> np.ndarray:
        """
        Perform averaging
        ...
        ----------
        data: np.ndarray
            The data on which to perform averaging
    ...
    def process(self, initial_signal: np.ndarray, processed_signal: np.ndarray, gt_signals: np.ndarray = None) -> dict:
        """
        Perform analysis on the solution object
        ...
        gt_signals: np.ndarray
            The ground truth if any (optional)

        Returns
        -------
        dict_results: dict
            A dictionary with all the metrics namely median frequency, correlation (if ground truth), lag (if ground truth), and rmse (if ground truth)
        """
    ...
    def _flatten(self, data: np.ndarray) -> tuple:
        """
        Flatten the data
        ...
        -------
        tuple
            Return the flattened data and the original shape
    ...
    def _unflatten(self, flattened_data: np.ndarray, original_shape: tuple) -> np.ndarray:
        """
        Reshape flattened data to original shape
        ...
        original_shape: tuple
            The shape of the original data to return
    ...
    def _compare_signal(self, ground_truth_signal: np.ndarray, signal_to_compare: np.ndarray) -> tuple:
        """
        Compare two signal together

        Parameters
        ----------
        ground_truth_signal: np.ndarray
            The reference signal
        signal_to_compare: np.ndarray
            The signal to compare with

        Returns
        -------
        tuple
            The comparison with the pearson correlation, the lag and the rmse
    ...
    def _perform_frequency_analysis(self, original_signal, reduced_signal, ground_truth_signal=None) -> list:
        """_summary_
        ...
        reduced_signal : np.ndarray
            The processed signal
        ground_truth_signal : np.ndarray
            The ground truth if any. Defaults to None.

        Returns
        -------
        list
            The median frequency for each input
        """
